...
<head>
<title>Доходы по вводу по мерчантам</title>
...
<?php $h2 = '<h2>Доходы по вводу в разрезе мерчантов</h2>';
		echo $h2;
...
if (!$output_form){
    ...
    $query1 = "SELECT curr_ps, merch_id, merch_name, SUM(input_ps_curr), SUM(ps_input), SUM(ps_commis), SUM(convert_comm), SUM(income), SUM(profit)
	FROM invoices WHERE date BETWEEN '$date_begin' AND '$date_end' GROUP BY curr_ps, merch_id";
	...
	$table = '<table><tr><th>Валюта</th><th>ID мерч.</th><th>Мерчант</th><th>Зачислено мерчантам</th><th>Поступило в ПС</th>
