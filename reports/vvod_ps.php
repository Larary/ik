...
<head>
<title>Доходы по вводу по ПС</title>
...
<?php $h2 = '<h2>Доходы по вводу в разрезе платежных систем</h2>';
		echo $h2;
...
if (!$output_form){
    ...
    $query1 = "SELECT curr_ps, paysystem, SUM(input_ps_curr), SUM(ps_input), SUM(ps_commis), SUM(convert_comm), SUM(income), SUM(profit)
	FROM invoices WHERE invoices.date BETWEEN '$date_begin' AND '$date_end' GROUP BY curr_ps, paysystem";
	...
	$table = '<table><tr><th>Валюта ПС</th><th>Платежная система</th><th>Зачислено мерчантам</th><th>Поступило в ПС</th>
				<th>Комиссия ПС</th><th>Конвертация</th><th>Доход</th><th>Прибыль</th></tr>';	
	while ($row = mysqli_fetch_array($result1)) {
		$table .= '<tr><td style="text-align:left;">'.$row['curr_ps'].'</td>';
		$table .= '<td style="text-align:left;">'.$row['paysystem'].'</td>';
	...
	$query2 = "SELECT curr_ps, SUM(input_ps_curr), SUM(ps_input), SUM(ps_commis), SUM(convert_comm), SUM(income), SUM(profit)
	FROM invoices where invoices.date BETWEEN '$date_begin' AND '$date_end' GROUP BY curr_ps";
