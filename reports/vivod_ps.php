...
<head>
<title>Доходы по выводу по ПС</title>
...
<?php $h2 = '<h2>Доходы по выводу в разрезе платежных систем</h2>';
		echo $h2;
...
if (!$output_form){
    ...
    $query1 = "SELECT curr_ps, paysystem, SUM(withdr_ps_curr), SUM(ps_withdr), SUM(merch_get), SUM(ps_commis), SUM(convert_comm), SUM(income), SUM(profit)
	FROM withdraws WHERE date BETWEEN '$date_begin' AND '$date_end' GROUP BY curr_ps, paysystem";
	...
	$table = '<table><tr><th>Валюта ПС</th><th>Платежная система</th><th>Списано с кассы</th><th>Списано в ПС</th>
				<th>Получено мерчантом</th><th>Комиссия ПС</th><th>Конвертация</th><th>Доход</th><th>Прибыль</th></tr>';	
	while ($row = mysqli_fetch_array($result1)) {
		...
		$table .= '<td>'.number_format($row['SUM(merch_get)'],2,',','').'</td>';
